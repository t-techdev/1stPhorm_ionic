...
export class CustomFoodComponent implements OnInit, OnDestroy {
   ...
   * @param value
   */
  public changeMealIndex($event: CustomEvent) {
    this._mealIndex = 1 * $event.detail.value;
  ...
  private createFoodItemValue(): FoodItem {
    ...
    return <FoodItem>{
      ...
      fiber: this.calculateRelativeValueConsumed(
        consumedSizeMeasure, form.amount_consumed.amount, form.fiber, form.serving_size
      ),
      ...
      thumbnail: this._foodItem ? this._foodItem.thumbnail : ''
    };
  }
  ...
  private initForm(): void {
    ...
    this.form = this.formBuilder.group({
      ...
      calculatedFiber: this.formBuilder.control({ value: '', disabled: false }),
  ...
  private setFormThroughFoodItem(foodItem: FoodItem) {
    // Why is this even necessary? -njv
    if (foodItem.fiber && foodItem.amount_consumed.amount) {
      foodItem.serving_size.fiber = foodItem.fiber / foodItem.amount_consumed.amount;
    }
    this.initForm();
    ...
    this.form.get('fiber').setValue(foodItem.serving_size.fiber ? foodItem.serving_size.fiber : '');
    ...
    this.form.get('calculatedFiber').setValue(foodItem.fiber ? foodItem.fiber : '');
  ...
  private fixDecimal(value: number) : number {
  ...
  private calculateRelativeValueConsumed(
    ...
    if (this.isCustom || this.isTemplateFood) {
      if (consumedSize.measure === 'serving') {
        return this.fixDecimal(consumedAmount * macro);
      }

      return this.fixDecimal((consumedAmount / servingSize.amount) * macro);
    }
    ...
    return this.fixDecimal(newWeight * macro * perServingWeight);
  }
  ...
  private setCalculatedValues() {
    ...
    const fiber = this.calculateRelativeValueConsumed(
      consumedSizeMeasure, form.amount_consumed.amount, form.fiber, form.serving_size
    );
    this.form.get('calculatedFiber').setValue(
      (fiber * 1 === 0) ? '' : fiber
    );
  }
  ...
  private fixForSmallIosScreenSizeOpen(value: boolean) {
    if (this.platform.is('cordova') && this.platform.is('iphone')) {
      this.keyboard.hideFormAccessoryBar(value);
    }
  }
