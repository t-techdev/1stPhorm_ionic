...
export class MealTemplatesService extends BaseService {
  ...
  constructor(
    ...
    MealTemplatesService.SET_PLATFORM(platform);
    MealTemplatesService.SET_ROLLBAR(rollbar);
  }
  ...
  public all(): Promise<MealTemplate<FoodItem>[] | ErrorFormat> {
    return this.http.get<MealTemplate<any>[]>(
      ...
      .then(mealTemplates => mealTemplates.map(mealTemplate => {
        mealTemplate.items = mealTemplate.items.map(mealItem => mealItem.serving_size ? mealItem : this.foodItem.convertToFoodItem(mealItem));
        return mealTemplate;
      }))
