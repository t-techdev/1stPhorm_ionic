...
import { Goal, Sex } from '../../interfaces';
...
describe('NutritionCalculatorService', () => {
  ...
  it('should calculate macros properly', () => {
    const service: NutritionCalculatorService = TestBed.get(NutritionCalculatorService);
    let result = service.calculateAdvancedMacros(325, 230, 'Sedentary', Goal.Lose, Sex.Male, 'Fats');
    ...
    result = service.calculateAdvancedMacros(325, 230, 'Sedentary', Goal.Lose, Sex.Female, 'Fats');
    expect(result).toEqual({
      calories: 2530,
      protein: 230,
      fats: 134,
  ...
  it('should use different base amounts for men and women', () => {
    const service: NutritionCalculatorService = TestBed.get(NutritionCalculatorService);
    let result = service.calculateAdvancedMacros(120, 120, 'Sedentary', Goal.Lose, Sex.Male, 'Fats');
    expect(result.carbs).toEqual(125);
    result = service.calculateAdvancedMacros(120, 120, 'Sedentary', Goal.Lose, Sex.Male, 'Carbs');
    expect(result.fats).toEqual(50);

    result = service.calculateAdvancedMacros(120, 120, 'Sedentary', Goal.Lose, Sex.Female, 'Fats');
    expect(result.carbs).toEqual(100);
    result = service.calculateAdvancedMacros(120, 120, 'Sedentary', Goal.Lose, Sex.Female, 'Carbs');
  ...
  it('should use different add carb and protein modifiers for weight diff >100lbs', () => {
    ...
    let result = service.calculateAdvancedMacros(120, 120, 'Sedentary', Goal.Lose, Sex.Male, 'Fats');
    expect(result.carbs).toEqual(125);
    result = service.calculateAdvancedMacros(320, 120, 'Sedentary', Goal.Lose, Sex.Male, 'Fats');
    expect(result.carbs).toEqual(150);

    result = service.calculateAdvancedMacros(120, 120, 'Sedentary', Goal.Lose, Sex.Female, 'Carbs');
    expect(result.fats).toEqual(40);
    result = service.calculateAdvancedMacros(320, 120, 'Sedentary', Goal.Lose, Sex.Female, 'Carbs');
    expect(result.fats).toEqual(60);
  });

  it('should calculate carbs and fats for "Both"', () => {
    const service: NutritionCalculatorService = TestBed.get(NutritionCalculatorService);
    let result = service.calculateAdvancedMacros(220, 180, 'Sedentary', Goal.Lose, Sex.Male, 'Both');
    expect(result).toEqual({
      calories: 2200,
      protein: 180,
      fats: 80,
    ...
    result = service.calculateAdvancedMacros(120, 115, 'Sedentary', Goal.Lose, Sex.Female, 'Both');
    expect(result).toEqual({
      calories: 1200,
      protein: 115,
      fats: 39,
  ...
  it('should add additional protein for weight diff >100lbs', () => {
    const service: NutritionCalculatorService = TestBed.get(NutritionCalculatorService);
    let result = service.calculateAdvancedMacros(425, 230, 'Sedentary', Goal.Lose, Sex.Male, 'Fats');
    expect(result.protein).toEqual(255);
    result = service.calculateAdvancedMacros(325, 230, 'Sedentary', Goal.Lose, Sex.Male, 'Fats');
    expect(result.protein).toEqual(230);
  });

  it('should pass Ryans tests', () => {
    const service: NutritionCalculatorService = TestBed.get(NutritionCalculatorService);
    let result = service.calculateAdvancedMacros(175, 140, 'Light Activity', Goal.Lose, Sex.Male, 'Carbs');
    expect(result).toEqual({
      calories: 1925,
      protein: 140,
      fats: 50,
    ...
    result = service.calculateAdvancedMacros(208, 200, 'Very Active', Goal.Gain, Sex.Male, 'Carbs');
    expect(result).toEqual({
      calories: 3744,
      protein: 200,
      fats: 50,
    ...
    result = service.calculateAdvancedMacros(120, 115, 'Sedentary', Goal.Lose, Sex.Female, 'Carbs');
    expect(result).toEqual({
      calories: 1200,
      protein: 115,
      fats: 40,
    ...
    result = service.calculateAdvancedMacros(290, 180, 'Sedentary', Goal.Lose, Sex.Female, 'Fats');
    expect(result).toEqual({
      calories: 2160,
      protein: 205,
      fats: 93,
